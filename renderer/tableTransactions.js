  ...
  initialize(id, data) {
    ...
    $(id + " tbody").off("click").on("click", "td", function () {
      if ($(id).DataTable().cell(this).index().column == 1) {
        ...
        $("#txHash").attr("href", vsprintf("https://explorer.enix.ai/tx/%s", [rowData[3]]));
        $("#txFromAddress").html(rowData[4]);
        $("#txFromAddress").attr("href", vsprintf("https://explorer.enix.org/address/%s", [rowData[4]]));
        $("#txToAddress").html(rowData[5]);
        $("#txToAddress").attr("href", vsprintf("https://explorer.enix.ai/address/%s", [rowData[5]]));
        $("#txValue").html(web3Local.utils.fromWei(EthoUtils.toFixed(parseFloat(rowData[6])).toString(), "Enix"));
